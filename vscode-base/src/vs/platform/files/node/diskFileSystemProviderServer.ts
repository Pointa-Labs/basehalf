...
export abstract class AbstractDiskFileSystemProviderChannel<T> extends Disposable implements IServerChannel<T> {
	...
	call<TResult>(ctx: T, command: string, args: unknown[]): Promise<TResult> {
		...
		switch (command) {
			...
			case 'writeFileExclusiveAtomic': return this.writeFileExclusiveAtomic(uriTransformer, args[0] as UriComponents, args[1] as VSBuffer) as Promise<TResult>;
	...
	private writeFileExclusiveAtomic(uriTransformer: IURITransformer, _resource: UriComponents, content: VSBuffer): Promise<IStat> {
		const resource = this.transformIncoming(uriTransformer, _resource);

		return this.provider.writeFileExclusiveAtomic(resource, content.buffer);
	}
