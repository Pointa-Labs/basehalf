...
function emitOpenSettings(): void {
  BrowserWindow.getFocusedWindow()?.webContents.send(WINDOW_IPC_CHANNELS.menuOpenSettings);
}
...
function emitCloseTab(): void {
  BrowserWindow.getFocusedWindow()?.webContents.send(WINDOW_IPC_CHANNELS.menuCloseTab);
}
