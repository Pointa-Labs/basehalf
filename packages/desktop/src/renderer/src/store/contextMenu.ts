 ...
 *
 * Mirrors the Dialog store (components/Dialog.tsx): state lives in Zustand so
 ...
 * at the App root renders it. Point-anchored (cursor x/y), not element-anchored
...
export const useContextMenuStore = create<ContextMenuStore>((set) => ({
  ...
  show: (x, y, items) => {
    ...
    if (typeof window !== 'undefined') window.bh?.suppressNextNativeContextMenu?.();
    set({ open: true, x, y, items: cleaned });
  },
