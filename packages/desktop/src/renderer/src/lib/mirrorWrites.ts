 ...
 * `bh:run` would resolve it against the window's NEW bound root (planting e.g. an
 * old file path into the new workspace's `.bh/`). `flushAll()` drains the editors;
 * this latch covers the non-editor debounced viewport/position/focus writers. The
 * damage was minor + self-healing (pruneDangling on the new load), but the binding
 * model is meant to make this race-free, so we close it here rather than heal.
 *
...
/** Called by `reopenHere(...)` immediately before `window.bh.reopenWindow(...)`. */
