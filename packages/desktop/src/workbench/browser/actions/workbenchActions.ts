 ...
 * the global Cmd+N shortcut (registered by workbench contributions). Centralising it here means
...
export function defaultDemoPath(): string {
  return `${nativeHostService.homeDir || '/tmp'}/BaseHalf-Demo`;
}
...
export function tildifyPath(path: string): string {
  const home = nativeHostService.homeDir;
  if (!home) return path;
  if (path === home) return '~';
...
// Workspace management dialogs. Reachable from the File menu (Workbench wires the
