	...
	jsonSchema: {
		...
		items: {
			...
			properties: {
				...
				icon: { type: 'string', pattern: '^[a-z][a-z0-9-]*$', description: nls.localize('contributes.basehalfCardProjections.icon', 'Codicon name without the codicon- prefix.') },
				extensions: { type: 'array', uniqueItems: true, minItems: 1, items: { type: 'string', pattern: '^\\.[A-Za-z0-9]+$' } },
				order: { type: 'number', default: 100 },
				defaultPriority: { type: 'number' }
			},
			required: ['id', 'label', 'icon', 'extensions']
		}
	},
...
export class BaseHalfCardProjectionExtensionPointContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		this._register(baseHalfCardProjectionsExtensionPoint.setHandler((users, delta) => {
			for (const removed of delta.removed) {
				this.registrations.deleteAndDispose(removed.description.identifier.value.toLowerCase());
			}
			for (const added of delta.added) {
				this.registerExtension(added);
			}
			// A handler installed after extension scanning receives the complete
			// current set as delta.added. `users` is intentionally not replayed.
		}));
	}
		...
		for (const contribution of user.value) {
			...
			try {
				contributionStore.add(this.projectionRegistryService.registerProjection({
					...
					icon: `codicon-${contribution.icon}`,
					selector: { extensions: contribution.extensions },
