...
const SIDEBAR_VIEWS: readonly SidebarView[] = ['files', 'scm', 'search'];
...
export function parseSidebarView(value: string | null): SidebarView {
...
export const useLayoutStore = create<LayoutState>((set, get) => {
  ...
  const sidebarView0: SidebarView = ((): SidebarView => {
    try {
      return parseSidebarView(localStorage.getItem(VIEW_KEY));
    } catch {
      return 'files';
    }
  })();
