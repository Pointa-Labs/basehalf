...
import { noteOpenedFile } from '../../history/browser/recentFiles.js';
...
// reliably survives the bridge. See code/electron-sandbox/preload.ts.
  ...
  return {
      ...
      try {
        ...
        if (currentWs) {
          try {
            ...
            // the canvas. The focus/badge channels inject this window's bound root.
    ...
    addDroppedPaths: async (paths: readonly string[]) => {
      ...
      try {
        ...
        if (lastName !== null) {
          nativeHostService.notifyWorkspacesChanged(); // all registrations (incl. non-opened) → menus
    ...
    createDemo: async (path: string) => {
      ...
      try {
        ...
        const r = await workspaceService.createDemo(path);
    ...
    remove: async (name: string) => {
      ...
      try {
        ...
        nativeHostService.notifyWorkspacesChanged(); // rebuild Open Recent / Dock menu
        if (wasCurrent) {
