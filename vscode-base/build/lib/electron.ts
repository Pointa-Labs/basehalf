...
function fileResponse(filePath: string): Promise<Response> | Response {
...
const electronAssetResolver = electronFeed
	...
	: async ({ url, fileName }: { url: string; fileName: string }): Promise<Response> => {
		if (fileName === 'SHASUMS256.txt') {
			return fileResponse(path.join(root, 'build', 'checksums', 'electron.txt'));
		}
