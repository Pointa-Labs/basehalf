...
const MAX_WINDOW_KEY_LENGTH = 4096;
...
/**
 * Persisted window layout, keyed PER workspace so multiple windows each
 ...
 *   session-restore list. Launch restores each registered key in this list.
 */
...
function isSafeWindowKey(key: string): boolean {
  return (
    key.length <= MAX_WINDOW_KEY_LENGTH &&
    key !== '__proto__' &&
    key !== 'constructor' &&
    key !== 'prototype'
  );
...
function sanitizeOpenKeys(open: readonly unknown[]): string[] {
  const keys: string[] = [];
  const seen = new Set<string>();
  for (const key of open) {
    if (typeof key !== 'string' || !isSafeWindowKey(key) || seen.has(key)) continue;
    keys.push(key);
    seen.add(key);
  }
  return keys;
...
function coerceGeometry(raw: unknown): WindowState | null {
  ...
  if (!isPositiveFinite(r.width) || !isPositiveFinite(r.height)) return null;
  return {
...
function normalizeWindowStates(parsed: {
  ...
  if (parsed.windows && typeof parsed.windows === 'object') {
    ...
    for (const [key, raw] of Object.entries(parsed.windows as Record<string, unknown>)) {
      if (!isSafeWindowKey(key)) continue;
      const geom = coerceGeometry(raw);
      if (geom) windows[key] = geom;
    }
    const open = Array.isArray(parsed.open) ? sanitizeOpenKeys(parsed.open) : [];
  ...
  const key =
    typeof parsed.workspaceRoot === 'string' && isSafeWindowKey(parsed.workspaceRoot)
...
export function geometryFor(file: WindowStatesFile, key: string): WindowState {
  return Object.prototype.hasOwnProperty.call(file.windows, key)
    ? (file.windows[key] ?? defaultGeometry())
    : defaultGeometry();
}

// Read-modify-write of the single shared file must serialize, or two windows
// persisting at once would each read-then-write and lose the other's slot. A
// main-process promise chain is enough — only main writes this file.
...
export function saveWindowStateSync(
  ...
  const file = readWindowStatesSync(configDir);
  if (isSafeWindowKey(key)) file.windows[key] = state;
  file.open = sanitizeOpenKeys(open);
  mkdirSync(configDir, { recursive: true });
  writeFileSync(join(configDir, FILE_NAME), JSON.stringify(file, null, 2), 'utf8');
}

/**
 * Validate persisted state against current displays before BrowserWindow sees it:
 * cap oversized windows to a usable work area and nudge positions back onscreen.
 * This mirrors VS Code's window-state validation boundary and keeps corrupt JSON
 * from producing tiny, negative, or enormous native windows.
 */
export function clampToDisplays(state: WindowState, displays: Display[]): WindowState {
  const clean = coerceGeometry(state) ?? defaultGeometry();
  const workAreas = displays.map(usableWorkArea).filter((area): area is Rectangle => area !== null);
  if (workAreas.length === 0) return clean;
  const fallbackArea = workAreas[0];
  if (!fallbackArea) return clean;

  const windowRect: Rectangle = {
    x: clean.x ?? fallbackArea.x,
    y: clean.y ?? fallbackArea.y,
    width: clean.width,
    height: clean.height,
  };
  const targetArea = workAreas.find((area) => rectsOverlap(windowRect, area)) ?? fallbackArea;
  const width = Math.min(clean.width, targetArea.width);
  const height = Math.min(clean.height, targetArea.height);
  if (clean.x === undefined || clean.y === undefined) {
  ...
  return {
    ...
    x: clampNumber(
      clean.x,
      targetArea.x,
      targetArea.x + targetArea.width - Math.min(MIN_VISIBLE_PIXELS, width),
    ),
    y: clampNumber(
      clean.y,
      targetArea.y,
      targetArea.y + targetArea.height - Math.min(MIN_VISIBLE_PIXELS, height),
    ),
    width,
    height,
  };
...
function usableWorkArea(display: Display): Rectangle | null {
  if (isUsableRect(display.workArea)) return display.workArea;
  if (isUsableRect(display.bounds)) return display.bounds;
  return null;
}
