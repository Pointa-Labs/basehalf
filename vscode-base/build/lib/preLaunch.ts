...
async function isExpectedElectronInstalled(): Promise<boolean> {
	try {
		...
		return installedVersion === electronVersion && await isExpectedElectronProductInstalled();
	} catch {
		return false;
	}
}

async function isExpectedElectronProductInstalled(): Promise<boolean> {
	const product = JSON.parse(await fs.readFile(path.join(rootDir, 'product.json'), 'utf8')) as {
		readonly nameLong: string;
		readonly nameShort: string;
		readonly applicationName: string;
	};
	try {
		await fs.access(getExpectedElectronExecutablePath(product));
		return true;
	} catch {
		return false;
	}
...
function getExpectedElectronExecutablePath(product: { readonly nameLong: string; readonly nameShort: string; readonly applicationName: string }): string {
	switch (process.platform) {
		case 'darwin':
			return path.join(rootDir, '.build', 'electron', `${product.nameLong}.app`, 'Contents', 'MacOS', product.nameShort);
		case 'linux':
			return path.join(rootDir, '.build', 'electron', product.applicationName);
		case 'win32':
			return path.join(rootDir, '.build', 'electron', `${product.nameShort}.exe`);
		default:
