...
import { FileGlyph, badgeType } from '../../labels/FileGlyph.js';
import { color, font, layout, radius, space, transition } from '../../style/design.js';
import { PopoverSurface, usePopover } from '../../ui/primitives/Popover.js';
...
const HomeCrumb = ({
  ...
  const focusMenuItem = useCallback(
    (direction: 1 | -1 | 'first' | 'last'): void => {
      const buttons = Array.from(
        floatingRef.current?.querySelectorAll<HTMLButtonElement>('button:not(:disabled)') ?? [],
      );
  ...
  useEffect(() => {
    if (!open) return;
    const id = window.setTimeout(() => focusMenuItem('first'), 0);
    return () => window.clearTimeout(id);
  }, [focusMenuItem, open]);

  const onSwitcherKeyDown = (e: ReactKeyboardEvent): void => {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      focusMenuItem(1);
  ...
  return (
    <>
      ...
      {rootIsCurrent && open && (
        ...
        >
          <div onKeyDown={onSwitcherKeyDown}>
            {workspaces.map((ws) => (
              <SwitcherItem
                key={ws.name}
                name={ws.name}
            ...
            >
              ...
              <span style={{ fontSize: font.size.ui, color: color.textSecondary }}>
                Open folder…
              </span>
            </SwitcherRow>
          </div>
