 ...
 * exists); renderers mirror it via the update-state IPC push.
 */
...
import type { PrefsStore } from '../../storage/electron-main/prefsStore.js';
...
export class Updater {
  ...
  constructor(
    ...
    opts: { readonly isPackaged?: () => boolean } = {},
  ) {
    this.isPackaged = opts.isPackaged ?? (() => app.isPackaged);
  }
  ...
  private setState(next: UpdateState): void {
    ...
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) win.webContents.send(UPDATE_IPC_CHANNELS.state, next);
    }
  }
  ...
  async install(): Promise<void> {
    ...
    try {
      if (!this.isPackaged()) {
        throw new Error('Updates are only available in the packaged BaseHalf app.');
      }
 ...
 *  next launch. */
export function cleanupUpdateLeftovers(opts: { readonly isPackaged?: () => boolean } = {}): void {
