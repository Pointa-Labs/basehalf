...
import { color, radius, shadow, space } from '../../style/design.js';
import { focusElementSafely } from '../focus.js';
...
export interface PopoverController {
  ...
  readonly close: (options?: PopoverCloseOptions) => void;
...
export const usePopover = ({
  ...
  const close = useCallback((options?: PopoverCloseOptions): void => {
    setOpen(false);
    if (options?.restoreFocus === true) {
      queueMicrotask(() => focusElementSafely(triggerRef.current));
    }
  }, []);
  ...
  useEffect(() => {
    if (disabled && open) close({ restoreFocus: true });
  }, [close, disabled, open]);
