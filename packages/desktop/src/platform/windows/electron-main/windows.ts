 ...
 * replacement for a process-global "current workspace" pointer: explicit IPC
 * channels read the sender window's bound root and inject it into their main
 * services, so every operation runs on THIS window's workspace.
 *
 ...
 * so no in-flight operation from the old page can observe a changed root.
 *
 ...
 * welcome/empty window (no workspace bound; workspace-scoped channels reject it).
 */
...
// ---------------------------------------------------------------------------
// Pure multi-window routing decisions. Kept free of BrowserWindow so they're
// unit-testable; the window router service performs focus/create/reload using
 ...
 *
 * Mirrors the welcome contribution's `recentWorkspacesModel.sortByRecency`
 * across the process boundary (both consume the same registry `lastOpenedAt`) —
 * a deliberate small duplication rather than reaching across main↔renderer.
 */
